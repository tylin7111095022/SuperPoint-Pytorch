...
def box_nms(prob, size=4, iou=0.1, min_prob=0.015, keep_top_k=-1):
    ...
    pts = torch.stack(torch.where(prob>=min_prob), dim=0).t() #  numKeyPoints*2 x 1 (y1 y2 ...... x1 x2 ......)
    boxes = torch.cat((pts-size/2.0, pts+size/2.0),dim=1).to(torch.float32) # left upper corner and right lowwer corner (y1, x1, y2, x2) shape is numKeyPoints*2 x 2
